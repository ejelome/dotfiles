...
1. Resolve `<route>` from the first token after `/quality`; `assess` and `show` consume the next token as their target. If missing or invalid, **ABORT** naming the token received.
