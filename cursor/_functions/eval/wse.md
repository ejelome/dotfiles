...
**Slash:** `/eval wse`
**Signature:** `/eval wse <project>`
**Phrases:** web stack review, WSE, principal web engineer (slash preferred).
...
- **Rubric** (walk items 1–9; skip what does not apply):
  ...
  - **Process** — map entrypoints; read root **`OWNERS.md`** if present when boundaries unclear; if `OWNERS.md` is absent and boundaries are unclear, ask `/eval ops` to generate it; missing router/CI = note, not automatic failure; **default** one topic per round; **`full`** or user "full pass" → all items with file anchors; prefer incremental fixes. `/eval tune` may aggregate this pass with other principal reviews.
