...
5. If `<role>` equals registry `moderatorRole`, **ABORT**: moderator cannot be removed by `/collab kick`; replace the moderator first.
6. If `<role>` equals registry `reviewerRole`, **ABORT**: reviewer cannot be removed while assigned; run `/collab set reviewer --clear` first to remove the reviewer assignment, then re-run `/collab kick`.
...
- **Parameters:** target collab slug, id, or numeric `#N` as the first token after `kick`; when absent, resolved per **Registry targeting** in **Notes**. `<role>` — required participant key to remove.
