...
11. Run scoped validation for the executing role. Minimum scoped validation is `SKIP_TESTS_RUN=1 ./tools/smoke-check.sh`; add targeted tests for touched behavior when available. Do not run `./tests/run.sh` for ordinary role-scoped re-execution. If the retried item is the terminal full-suite Action Plan item, run the full sequence instead: `SKIP_TESTS_RUN=1 ./tools/smoke-check.sh` → `./tests/run.sh` → `./link.sh` → `SMOKE_CHECK_RUNTIME=1 SKIP_TESTS_RUN=1 ./tools/smoke-check.sh`.
