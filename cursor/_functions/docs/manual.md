...
**Slash:** `/docs manual`
**Signature:** `/docs manual`
**Phrases:** `update manual`, `create manual`, `manual fallback`, `recovery guide`, `MANUAL.md`
...
2. If **`shared-docs-toc.mdc`**, **`shared-docs-precedence.mdc`**, or **`auto-docs-markdown.mdc`** is required for formatting and is not readable, **ABORT** per **`auto-context-gate.mdc`**.
...
7. Use imperative mood for steps; sentence-case headings; fenced blocks with language tags; plain Markdown without decorative horizontal rules except an optional TOC band per **`shared-docs-toc.mdc`**.
8. On **Update**, re-trace changed scripts, sync headings and steps, sync TOC if present, refresh **Status** when meaning changes.
9. Before finishing, verify each of the following. Every numbered step and fence maps to a traced source. Filename mentions in step sentences are limited to the replaced automation or traced optional helpers. The **Status** date follows **Date sourcing** in **Notes**. The TOC lists every `##`, `###`, and deeper heading, or follows the label-only rule from **`shared-docs-toc.mdc`**. Markdown structure and voice follow **`auto-docs-markdown.mdc`** through **`shared-docs-precedence.mdc`**. No edits exist outside repo-root **`MANUAL.md`**.
...
- **README pairing:** README often carries orientation and short install; **`MANUAL.md`** carries traced fallback steps. Do not invoke **`/docs readme`** from this command. Link normally when the repo already cross-links.
