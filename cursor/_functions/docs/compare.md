...
Compare original versus refined Markdown for preservation of facts, headings, and constraints; do not emit a compact final (use **`/docs compact`** after).

## Trigger

**Slash:** `/docs compare`
**Signature:** `/docs compare <path1> <path2>`
**Phrases:** `compare markdown`, `compare versions`, `compare two docs`, `preservation check`, `diff markdown`
...
5. State whether the refined text is a reasonable base for compaction. Point to **`/docs compact`** when that is the logical next step.
