...
Contract: [style.md](style.md#llm-consumed-files), [document.md](document.md#llm-consumed-files), [command.md](command.md)
...
Determine the rule’s scope before writing a single instruction. A rule that applies to every file in every context is a token tax paid on every turn. Use globs to limit application to the files and paths the rule actually governs. Use `alwaysApply: false` for everything that is not genuinely universal.
...
Example (better) — shape matches a TypeScript-scoped `.mdc` rule such as `auto-code-typescript.mdc` (illustrative):
...
Rules (`.mdc`) are persistent instructions the model applies when the rule’s scope matches the current context. The model follows the text literally. The model does not infer intent.

Writing standards — instruction format, ambiguity prevention, conflict resolution, and labeled examples — are defined in the [style guide](style.md#llm-consumed-files).
...
- **`auto-` rules (main):** set either `alwaysApply: true` and omit `globs`, or `alwaysApply: false` with path `globs` — never both. Each `auto-` rule is active globally when always-on, or when its globs match; it is not a dependency-only helper.
- **`shared-` rules (dependency):** keep `alwaysApply: false`. Load via Agent Requested, path `globs`, or references from other rules — they do not replace a main `auto-` rule for a domain.
- State the condition before the instruction in any conditional rule. Example from `auto-code-typescript.mdc` (illustrative): “Applies when editing or suggesting `*.ts` and `*.tsx` in any repository (front matter globs).” Then follow with imperative behavior lines such as “Apply on every edit” and “Never use `any`.”
...
Catalog sync, **Trigger**/**Steps**/**Notes** order, slash **H1** alignment, and link hygiene for **`commands/*.md`** and **`_functions/**/*.md`** live in [command standard](command.md).

### Precision

- Reference exact file paths, not vague descriptions: `rules/{auto,shared}.mdc`, `_mdc/{auto,shared}/*.mdc`, and `_functions/docs/readme.md` in the Cursor config tree, not “the rules folder” or “the readme command”
...
Skills (`SKILL.md`) load when the model or user selects them. Apply the same signal, scope, and one-action-per-step discipline as commands. Per-type shape lives in the [document standard](document.md#llm-consumed-files). Skills are delivered and versioned by the editor host; paths are not part of the `_core/` canon.
...
- Keep every `*.md` and `*.mdc` under the Cursor config root within the **File size discipline** limits below; split or trim when a file outgrows them
- Prefer path-scoped activation (`alwaysApply: false` with `globs` on an `auto-` rule, or a `shared-` dependency) over an `auto-` rule with `alwaysApply: true` when the guidance applies only to some paths or tasks — see [file naming](style.md#file-naming)
- Remove instructions that duplicate what ESLint, TypeScript, Prettier, or the repo test runner already enforce when those tools are authoritative — those constraints are free at inference time (see `auto-code-typescript.mdc` in the rules layer, illustrative, for the stack that tree assumes)
...
`SKILL.md` files live under the host skills directory (not under `CURSOR_CONFIG_ROOT` in the default layout); length and split guidance for skills live in the [document standard](document.md#llm-consumed-files). Non-markdown manifests are outside this Markdown line budget.
...
- [Style guide — file naming](style.md#file-naming) — naming conventions for rules and commands, front-matter requirements.
- [Style guide — LLM-consumed files](style.md#llm-consumed-files) — writing standards: imperative instructions, ambiguity, conflict resolution, examples.
- [Document standard](document.md#llm-consumed-files) — per-type templates for rule (`.mdc`), command (`.md`), and skill (`SKILL.md`) shape.
- [Command standard](command.md) — extended **`commands/`** playbook contract.
- [Author voice](voice.md) — personal-account voice.
