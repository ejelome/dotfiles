...
The command standard defines how public slash files under `commands/` delegate to private function playbooks under `_functions/` in `CURSOR_CONFIG_ROOT`. Every playbook follows the [style guide](style.md) and the **Command** (`.md`) template in [document standard](document.md#command-md); this file owns the extended contract: public slash routing, private functions, heading order, catalog duties, link hygiene, harness exceptions, and layout rules. Signal, one-action-per-step discipline, and the **250-line** budget live in [context engineering](context.md).

Contract: [document.md](document.md#command-md), [style.md](style.md), [context.md](context.md)

## Layering

- **Minimal template** — opening sentence plus `## Trigger`, `## Steps`, `## Notes` in [document standard](document.md#command-md).
- **This standard** — operational details below, including namespace routing, private functions, and catalog synchronization.
- **Voice** — neutral imperative for machine steps; [author voice](voice.md) only when the playbook deliberately uses personal register.
...
- The first body paragraph is one declarative sentence that starts with the action verb and states purpose and when to open the playbook, per [document standard](document.md). Only eval private functions may use the bold-acronym-first opening described there.
...
- Lead with `**Slash:** /<namespace>` for public namespace files or `**Slash:** /<namespace> <route>` for private functions.
- Required route choices and positional arguments are declared in the **Signature** line.
- Add `**Phrases:**` after `**Slash:**` or after `**Signature:**` when one is present.
- For multi-mode functions, use a labeled group per mode: `**Phrases (mode):**`.
...
Commands either take no arguments or every listed argument is required. There is no optional-argument notation.
...
**Phrases:** <natural-language alias>, <alias>
...
- Obey the **250-line** cap for `commands/*.md`, `_functions/**/*.md`, and command harness docs documented under **File size discipline** in [context engineering](context.md).
...
- [Style guide](style.md) — voice, ambiguity, LLM-consumed rules, file naming.
- [Document standard](document.md) — minimal command template and other shapes.
- [Context engineering](context.md) — phases, precision, context budget.
- [Author voice](voice.md) — register for personal-account exceptions.
