...
- Reference exact file paths, not vague descriptions: `rules/{auto,shared}.mdc`, `_mdc/{auto,shared}/*.mdc`, and `_functions/doc/write-readme.md` in the Cursor config tree, not “the rules folder” or “the readme command”
...
- Prefer path-scoped activation (`alwaysApply: false` with `globs` on an `auto-` rule, or a `shared-` dependency) over an `auto-` rule with `alwaysApply: true` when the guidance applies only to some paths or tasks — see [file naming](style-guide.md#file-naming)
...
- [Document standard](document-standard.md#llm-consumed-files) — per-type templates for rule (`.mdc`), command (`.md`), and skill (`SKILL.md`) shape.
