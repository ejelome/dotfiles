...
The style guide applies to all `.md` and `.mdc` files across all projects and notes, except where [Personal account exception](#personal-account-exception) or [Changelog entries](#changelog-entries) says otherwise. Per-type templates for common shapes live in [the document standard](document.md); slash playbook details live in [command standard](command.md).

Contract: [document.md](document.md), [command.md](command.md), [context.md](context.md)
...
- Personal accounts (see Personal account exception): person follows [the author voice guide](voice.md) — often first person singular (`I`), not neutral documentation
...
- The first sentence of every document is a single declarative statement. Never open with “This document…”, “Welcome to…”, or “This guide covers…”. Exception: `eval/*.md` route playbooks may open with a bold acronym label as the subject — see the **Command** (`.md`) template in [document standard](document.md) for the exact shape.
...
- `H1` — document title only, one per file, noun phrase by default; slash playbooks and harness files follow their own title shape in [command standard](command.md)
- `H2` — major sections
...
Personal accounts — devblog entries, personal essays, journaling, public longform (blogs, feeds, essays), and narrative drafts — follow [the author voice guide](voice.md) instead of the standard voice rules. The formatting rules (headings, lists, code, links) still apply. Person and tense follow the voice guide (including first person where it is the default). Imperative mood does not apply.

## Changelog entries

`CHANGELOG.md` body lines are often generated from commit messages or sync scripts. Neutral tone and the person rules above do not apply to those machine-sourced lines. Section layout follows [the document standard](document.md) and [Keep a Changelog](https://keepachangelog.com/en/1.1.0/).
...
|`group`|`docs`, `cmd`, `git`, `eval`, `review`, …|Domain that ties the rule to its command group. Matches the group prefix of related commands.                                                                                                                                           |
|`name` |kebab-case noun                          |What the rule governs within the group.                                                                                                                                                                                                 |

Examples: `shared-docs-voice.mdc`, `auto-docs-markdown.mdc`, `shared-cmd-eval.mdc`
...
Examples: `_functions/git/commit.md`, `_functions/eval/igd.md`, `_functions/docs/readme.md`
...
- [Document standard](document.md) — per-type templates for each file shape.
- [Command standard](command.md) — extended slash playbook rules.
- [Author voice](voice.md) — voice, register, and craft principles for personal accounts.
- [Context engineering](context.md) — context engineering principles and context budget guidance for rules and commands.
