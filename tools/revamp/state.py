...
def validate_state(data: dict, source: Path) -> None:
    ...
    for field in ('roleBindings', 'concernRequirements', 'phaseOutputs'):
...
def base_state(repo_root: Path, stage: str, validation_commands: list[str]) -> dict:
    ...
    return {
        ...
        'roleBindings': {},
        'concernRequirements': DEFAULT_CONCERN_REQUIREMENTS,
        'phaseOutputs': {},
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Revamp narrative state helper.')
