...
grep -Fq 'Ask for a single yes/no confirmation before writing anything' "$file" || fail "upgrade.md: missing single confirmation gate"
grep -Fq 'If the user declines or no confirmation is received, leave all files and the marker untouched' "$file" || fail "upgrade.md: missing no-confirmation unchanged-files guarantee"
