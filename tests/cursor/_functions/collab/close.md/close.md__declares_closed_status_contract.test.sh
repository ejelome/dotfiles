...
grep -Fq 'Subsequent routes must refuse target inference until the moderator runs `/collab activate <record>`' "$file" || fail "close.md: missing registry cleanup fallback note"
...
grep -Fq '`/collab write summary` remains available on closed records' "$file" || fail "close.md: missing summarize-after-close rule"
