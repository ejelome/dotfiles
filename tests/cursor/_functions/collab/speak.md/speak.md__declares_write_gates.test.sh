...
grep -Fq 'Unless `--verbatim` is present, the helper applies the structure-only polish transform defined in `cursor/_functions/collab/_moderator-polish.md`' "$file" || fail "speak.md: missing moderator formatting boundary"
...
grep -Fq 'writes on the `speak-render` path are limited to the registry and transcript under `.collabs/`' "$file" || fail "speak.md: missing execution boundary"
grep -Fq 'Use `/collab retract speak` to replace the current role' "$file" || fail "speak.md: missing retract recovery path"
grep -Fq 'Moderator-role formatting is helper-owned by `cursor/_functions/collab/_moderator-polish.md`' "$file" || fail "speak.md: missing collab formatting rule link"
