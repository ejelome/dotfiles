...
grep -Fq 'returns the active phase, effective turn order, active-phase contributors, last live contributor, expected role, and auto-advance exemption state' "$file" || fail "speak.md: missing helper-owned state surface"
...
grep -Fq 'Never perform any file edit, shell command, or codebase change as a side effect of this command' "$file" || fail "speak.md: missing execution boundary"
grep -Fq '[auto-collab-format](../../_mdc/auto/auto-collab-format.mdc)' "$file" || fail "speak.md: missing collab formatting rule link"
