...
grep -Fq 'If the active phase is not `Completion`, **ABORT**: `/collab execute` is valid only when `**Active phase:** Completion`.' "$file" || fail "execute.md: missing Completion-only guard"
...
grep -Fq 'If `**Execution:** completed by <role>` already exists, report that the role'\''s execution is already complete and stop.' "$file" || fail "execute.md: missing role completion idempotency guard"
grep -Fq 'Append `**Execution:** in progress by <role>` under `## Completion`.' "$file" || fail "execute.md: missing in-progress execution trace"
...
grep -Fq 'Append `**Execution:** completed by <role> — YYYY-MM-DD` under `## Completion` after validation passes.' "$file" || fail "execute.md: missing completion trace"
grep -Fq 'If validation fails, append `**Execution:** failed by <role> — YYYY-MM-DD` with the failed command before reporting.' "$file" || fail "execute.md: missing failure trace"
grep -Fq 'target collab slug, id, or legacy transcript path as the first token after `execute`; when absent, resolved per **Registry targeting** in **Notes**.' "$file" || fail "execute.md: missing registry target parameter"
