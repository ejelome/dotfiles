...
use_file="$ROOT/cursor/_functions/collab/activate.md"
open_file="$ROOT/cursor/_functions/collab/open.md"
kick_file="$ROOT/cursor/_functions/collab/remove-participant.md"
...
grep -Fq '**Signature:** `/collab activate <record>`' "$use_file" || fail "use.md: missing use signature"
...
grep -Fq '**Signature:** `/collab remove participant <role>`' "$kick_file" || fail "kick.md: missing kick signature"
...
grep -Fq 'moderator cannot be removed by `/collab remove participant`' "$kick_file" || fail "kick.md: missing moderator protection"
...
grep -Fq 'Never skip the gate prompt' "$delete_file" || fail "delete.md: missing confirmation required"
