...
grep -Fq 'cursor/_core/agent-role.md' "$file" || fail "join.md: missing role schema"
grep -Fq '`key` (must match `<role>`), `displayName`, `concerns`' "$file" || fail "join.md: missing role schema"
grep -Fq 'Optional field: `prohibitions` (string array; principle-level behavioral constraints, not a runtime enforcement list).' "$file" || fail "join.md: missing optional prohibitions schema"
