...
grep -Fq 'Validate the role JSON against [cursor/_core/agent-role.md](../../_core/agent-role.md): `key` must match `<key>`, and `displayName` and non-empty `concerns` must be present.' "$file" || fail "narrative.md: missing role schema validation"
grep -Fq 'If invalid, **ABORT**: invalid role JSON; name the failed field.' "$file" || fail "narrative.md: missing invalid role abort"
grep -Fq 'it writes the resolved key to `roleBindings[phase]`' "$file" || fail "narrative.md: missing roleBindings write contract"
...
echo "PASS: narrative/rewrite-content.md declares role resolution contract"
